...
#
#    This file is part of the SHARC-GYM.
#
#    SHARC-GYM is free software: you can redistribute it and/or modify
#    it under the terms of the GNU General Public License as published by
#    the Free Software Foundation, either version 3 of the License, or
#    (at your option) any later version.
#
#    SHARC-GYM is distributed in the hope that it will be useful,
#    but WITHOUT ANY WARRANTY; without even the implied warranty of
#    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
#    GNU General Public License for more details.
#
#    You should have received a copy of the GNU General Public License
#    from the git repository.  If not, see <http://www.gnu.org/licenses/>.
#
#    This file is adapted from a SHARC2.1 script using the same license. 
#    See https://github.com/sharc-md/sharc.
#
#******************************************
...
def close_keystrokes():
  KEYSTROKES.close()
  if GYM:
    shutil.move('KEYSTROKES.tmp','KEYSTROKES.setup_init_gym')    
  else:
...
def get_general():
  ...
  if GYM:
    states=[1,0,1] #dummy states, real number is read from LVC.template
  else:  
  ...
  if 'overlap' in Interfaces[num]['features']:
    ...
    if GYM:
      INFOS['refov']=question('Reference overlaps?',bool,True) 
    else:   
...
def get_LVC(INFOS):
  ...
  if GYM:
    ...
    template = open('LVC.template', 'r')
    template.readline()
    states = [ int(x) for x in template.readline().split() ]
    print states
    nstates=0
    for mult,i in enumerate(states):
      nstates+=(mult+1)*i
...
'''
  ...
  parser.add_option('--sharc_gym', dest='GYM', action='store_true',help="Reduced input for SHARC_gym setups")
